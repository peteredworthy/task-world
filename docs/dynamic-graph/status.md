...
- FR-02/FR-03/FR-04/FR-05/FR-14/FR-17: authority and approval decision records,
  ...
  plus less-used node/record families such as recovery and terminal failure
  paths.
...
- FR-12/FR-16: heartbeat, terminal cancellation, late stale-callback rejection,
  ...
  guard without duplicate-position/SQLite-lock failure, plus terminal
  `failure_record` callback behavior after exhausted runtime failure.
- FR-15: worktree file-state capture, tool-cache staging exclusion,
  ...
  and failed/revoked write-lease cleanup remain
  product-unproven.
...
For FR-16, this means an acceptance suite should cover the already-proven
supported-runner paths (submit, grade, patch, heartbeat, artifact/output,
stale rejection, unsupported-runner fail-fast) and the still-open terminal
exhausted-failure/`failure_record` behavior. For FR-12, it should cover
durable recovery re-entry and stale-report guarding, not just ordinary
`agent_died` retry. This is a process correction: do not add another
partial-proof-only commit unless it first installs or extends the executable
row-level acceptance harness.
...
2. FR-12: extend the callback/recovery acceptance harness for recovery re-entry.
   Heartbeat is product-proven through runtime-owned graph dispatch events in
   `961b0f47...` and `1010af96...`; late callback rejection after terminal
   graph cancellation is product-proven in `1010af96...`; concurrent
   start-acknowledgement stale-position retry is product-proven in
   `7bd58637...`; supported-runner artifact/output callbacks are product-proven
   in `a9b658fa...`; and real `agent_died` failure/retry callbacks are
   product-proven in `32be2af2...`. FR-16 terminal exhausted-failure
   `failure_record` behavior is now covered by
   `test_fr16_terminal_exhausted_failure_record_callback_readbacks`. Still
   open for FR-12: a repeatable recovery re-entry acceptance test that
   exercises the stale-report guard with no duplicate-position/SQLite-lock
   failure.
3. FR-08/FR-11: after the acceptance harness exists, add bounded
   product probes for authority denial/rejection/revocation, hidden-command
   rejection, broader multi-ready fairness/starvation behavior, cleanup as
   explicit graph work, and failed/revoked write-lease cleanup beyond API
   cancel lease revocation. Product probe
   `961b0f47...` exposed
