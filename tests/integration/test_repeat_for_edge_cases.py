...
class TestRepeatForConditionPersistence:
    async def test_expanded_steps_persist_to_database(self, service: WorkflowService) -> None:
        ...
        for i, item in enumerate(["a", "b", "c"]):
            ...
            # Give each task copy a unique ID to avoid SQLAlchemy identity-map collisions
            # when multiple steps (all deepcopied from the same original) are saved together.
