...
| 5 | Node-expectation redesign | Evidence requirements cannot be poisoned by one brittle node identity when producer-class evidence satisfies the intent. | Fixed | `tests/unit/test_graph_commands.py::test_callback_binds_required_input_by_wildcard_producer_class_edge` and `tests/unit/test_graph_horizon_templates.py::test_final_invariant_template_uses_wildcard_verifier_evidence`. The default final-invariant template now wires required verification evidence through a wildcard producer-class edge (`from_node_id: "*"`, `from_node_kind: "verifier"`, `from_node_role: "verifier"`) rather than the brittle `verifier-corrective-{region_id}` node identity. |
...
| 9 | `claude_sdk` graph submit fix | `claude_sdk` can submit graph callbacks through the same path as the reliable graph runner, or is explicitly gated off with a reproduction-backed reason. | Fixed | `tests/integration/test_graph_run_driver.py::test_driver_rejects_unsupported_graph_runner_before_seeding[claude_sdk-claude_sdk]` locks the closure path from the HTML review: `claude_sdk` graph runs are explicitly gated off before graph seeding with `graph_runner_unsupported`, leaving `codex_server` as the reliable graph runner. `tests/unit/test_claude_sdk_agent.py::test_build_prompt_ignores_graph_patch_callback` and `tests/unit/test_claude_sdk_agent.py::TestBuildOrchestratorMcpServer::test_submit_graph_patch_tool_is_not_registered` lock permanent retirement of the dormant SDK graph-callback path. |
