...
**Source:** `docs/single-queue-2/intent.md`, `docs/single-queue-2/plan.md`
...
Consumer loop (next poll cycle):
  ...
  → Call env_lifecycle.on_run_start()
  → SET handled_at = now()
```
...
| `processed_at` | Set when signal handled | Renamed to `handled_at` (set when handler returns OK) |
| `delivered_at` | — | `TIMESTAMP NULL` (new column, set when consumer dispatches) |

**Note:** The existing `processed_at` column is repurposed as `handled_at` (rename only).
Only `delivered_at` is a genuinely new column. This minimizes schema churn.
...
- **Polling interval:** 100ms (10 queries/second when idle; tunable if needed).
- **Concurrency model:** One `asyncio.Task` per `run_id`. Signals for the same run are processed serially inline. A slow operation (e.g., `env_lifecycle.on_run_start()` creating a git worktree) blocks subsequent signals for that run but does not block other runs.
- **Crash recovery:** On startup, signals with `delivered_at IS NOT NULL AND handled_at IS NULL` for inactive runs are re-dispatched through normal handlers.
...
these methods now return immediately after enqueueing. Start/pause/resume/cancel
endpoints change from **200 to 202 Accepted** (breaking change — decided in
clarifications). The API does not wait for the consumer to process the signal.
Frontend code must be updated to handle 202 responses from these endpoints.
...
The frontend already polls for run status updates. The `STOPPING` state is exposed
in the REST API and frontend (full transparency, decided in clarifications). It needs
to be added to the frontend `RunStatus` type and displayed with a "Stopping..." indicator.
Although STOPPING is transient (seconds), the frontend may occasionally observe it between
polls. This is a minor UI change.
...
| Migration on production DB with in-flight signals | Migration assumes clean server stop with no in-progress runs. No backfill of in-flight signals needed. |
