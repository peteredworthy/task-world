"""Unit tests for the public events_v2 event store surface."""
...
class _RecordingV2Store:
    def __init__(self, calls: list[str], *, fail: bool = False) -> None:
        self.calls = calls
        self.fail = fail
        self.events: list[WorkflowEvent] = []

    async def append(self, events: WorkflowEvent | Sequence[WorkflowEvent]) -> list[Any]:
...
def test_legacy_event_store_is_not_public_db_api() -> None:
    assert "EventStore" not in db.__all__
    assert not hasattr(db, "EventStore")
    with pytest.raises(ImportError):
        exec("from orchestrator.db import " + "Event" + "Store", {})
...
async def test_persistent_emitter_notifies_listener_without_duplicate_append() -> None:
    calls: list[str] = []
    store = _RecordingV2Store(calls)
    emitter = PersistentEventEmitter(store)
...
async def test_persistent_emitter_blocks_listeners_on_store_failure() -> None:
    calls: list[str] = []
    store = _RecordingV2Store(calls, fail=True)
    emitter = PersistentEventEmitter(store)
    received: list[WorkflowEvent] = []
    emitter.add_listener(received.append)

    with pytest.raises(RuntimeError, match="v2 append failed"):
        await emitter.emit(_run_event())
