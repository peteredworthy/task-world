...
record envelope/readback, and region acceptance semantics. FR-03 and FR-17 keep
the new evidence but remain partial because those rows still include broader
less-used node contract/readback families outside this final-gate pass.

### 2026-06-25 FR-12 Acceptance Harness Refresh

Product-scope FR rows remain `partial` except FR-04, FR-05, FR-12, FR-14, and
FR-16, which now have row-level executable acceptance coverage plus calibrated
product-path evidence, and FR-19, which is explicitly out of scope. No
additional row should be promoted to `validated` until every acceptance
...
- FR-02/FR-03/FR-17: authority and approval decision records,
  ...
  now acceptance-harness-proven. Remaining proof is narrower: less-used
  node/record/readback families such as recovery and terminal failure paths
  where the row scope still asks for them.
...
1. FR-03/FR-17: native accepted authority-request decision readbacks are now
   ...
   state all came from the product graph/API path. Explicit `final_gate`
   completion-rule/detail semantics are now covered by
   `tests/integration/test_graph_fr14_final_gate_acceptance.py::test_final_gate_completion_decision_and_region_readbacks`.
   Remaining FR-03/FR-17 proof is narrower: broader less-used node kinds and
   readback families not covered by the authority, callback, failure, and
   final-gate harnesses. Actual pending gate, approval/authority decision
...
2. FR-08/FR-11: after the acceptance harness exists, add bounded
