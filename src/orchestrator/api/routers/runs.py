...
def _token_usage_to_schema(usage: Any) -> ModelTokenUsageSchema:
...
def _action_log_to_schema(action_log: Any | None) -> ActionLogSchema | None:
...
def _attempt_to_schema(attempt: Any) -> AttemptSchema:
    return AttemptSchema(
        ...
        token_usage_by_model=[
            _token_usage_to_schema(usage) for usage in attempt.token_usage_by_model
        ],
...
def _run_to_response(run: Run) -> RunResponse:
    ...
    token_usage_schemas = [_token_usage_to_schema(u) for u in run.token_usage_by_model]
...
def _run_to_trace_response(run: Run) -> RunTraceResponse:
    trace_attempts: list[RunTraceAttempt] = []
    for step_index, step in enumerate(run.steps):
        for task in step.tasks:
            for attempt in task.attempts:
                action_log_schema = _action_log_to_schema(attempt.action_log)
                ...
                trace_attempts.append(
                    RunTraceAttempt(
                        ...
                        attempt=_attempt_to_schema(attempt),
                        phases=phases,
    ...
    return RunTraceResponse(
        ...
        total_duration_ms=run.total_duration_ms,
        total_num_actions=run.total_num_actions,
        token_usage_by_model=[_token_usage_to_schema(u) for u in run.token_usage_by_model],
...
async def get_run_trace(
    ...
    """Get all task attempts and structured action logs for a run."""
    run = await service.get_run(run_id)
    return _run_to_trace_response(run)
