...
| `ResourceClaimProjection` | already sufficiently constrained by Pydantic | Strict canonical shape | Accepts canonical `paths` only. A malformed legacy claim using singular `path` fails model validation and is dropped when checkpoint state is restored; no legacy path normalization occurs. |
...
| `NodeCreationProjection.resource_claims` | direct copied event payload | Strict typed list | Exactly `list[ResourceClaimProjection]`; claims use canonical `paths` and have no legacy path normalization. |
