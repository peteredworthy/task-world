...
| F1 | Recognized cache directories are represented as bounded roots. | `.venv`, `node_modules`, and other policy-recognized cache trees produce only a root record; their descendants remain uninspected and emit no path or security evidence. Tracked source under mixed roots remains visible, while a cache-root symlink and descendants of unknown directories retain secret/repo-escape security checks. | Isolated 1,500-file `node_modules` proof emits exactly the cache root while leaving its secret and escaping-symlink descendants uninspected; separate root-symlink and unknown-directory proofs reject unsafe paths. | Large-cache proof and focused file-state/security suites pass. | Validated | None. |
