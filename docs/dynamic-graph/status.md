...
The FR proof-chasing plan is closed as of 2026-06-26. Historical remaining-work
material remains in `docs/dynamic-graph/remaining-fr-validation-plan.html` and
`docs/dynamic-graph/complete/`, but the table below is the current FR ledger:
FR-01 through FR-18 are validated, and FR-19 is explicitly out of scope. The
...
for graph execution and its dormant SDK graph-patch prompt/MCP path has been
removed, so the active supported-runner list is `codex_server`.
...
Product-scope FR rows FR-01 through FR-18 now have row-level executable
acceptance coverage plus calibrated product-path evidence where live runner
behavior was required. FR-19 remains explicitly out of scope. No future row or
new criterion should be promoted to `validated` until every acceptance
criterion has product-real proof or deterministic graph-runtime/API harness
coverage through `/api/runs`, graph workflow execution, runtime callbacks, and
graph API/readback paths.

The final proof-closure clusters are:
...
### 2026-06-22 Validation Strategy Correction (Archived)

At the time of this correction, the remaining partial rows were too broad to
close reliably with one-off manual product probes. Manual probes remained useful
for discovering product bugs and calibrating expected API/event shapes, but they
were no longer the primary validation mechanism for promoting FR rows to
`validated`. This process note is retained as historical guidance; the row table
below supersedes its earlier partial-row state.

The adopted rule was: before another row-promotion attempt, add an executable
FR acceptance harness that encodes the row's full acceptance criteria as
repeatable checks against the product graph surfaces: `/api/runs`, graph
execution mode, runtime callbacks, `/graph/events`, node detail, scheduler
readbacks, regions, final blockers, and public run state. A row may move from
`partial` to `validated` only when:
...
FR-16 and FR-12 are now closed under this rule. FR-16 covers the already-proven
supported-runner paths (submit, grade, patch, heartbeat, artifact/output, stale
rejection, unsupported-runner fail-fast) and terminal
exhausted-failure/`failure_record` behavior. FR-12 covers durable recovery
re-entry and stale-report guarding, not just ordinary `agent_died` retry. This
process correction remains in force for any future reopened or newly added FR:
do not add a partial-proof-only commit unless it first installs or extends the
executable row-level acceptance harness.
...
## Archived Proof-Closure Set

The smallest incomplete proof set is closed. This archived list records the
acceptance harnesses that moved the final product-scope rows to `validated`.
For future reopened or newly added criteria, follow the same pattern: install or
extend a repeatable FR acceptance harness first, then use bounded product probes
only to calibrate live-runner evidence that cannot be reproduced
deterministically in-process.

0. Acceptance harness rule: create executable row-level acceptance tests for the
