...
Product-scope FR rows remain `partial` except FR-04, FR-05, FR-08, FR-11,
FR-12, FR-14, and FR-16, which now have row-level executable acceptance
...
2. FR-15: after the acceptance harness exists, add bounded product probes for
   cleanup as explicit graph work and failed/revoked write-lease cleanup beyond
   API cancel lease revocation. Product probe
   `961b0f47...` exposed
   ...
   readbacks. FR-08 is now validated by
   `tests/integration/test_graph_fr08_acceptance.py::test_fr08_invalid_patch_matrix_rejected_and_readable`
   and
   `tests/integration/test_graph_fr08_acceptance.py::test_fr08_authority_denial_and_rejection_readbacks`
