...
Product-scope FR rows remain `partial` except FR-04, FR-05, FR-11, FR-12,
FR-14, and FR-16, which now have row-level executable acceptance coverage plus
calibrated product-path evidence, and FR-19, which is explicitly out of scope. No
...
2. FR-08/FR-15: after the acceptance harness exists, add bounded
   product probes for authority denial/rejection/revocation, hidden-command
   rejection, cleanup as explicit graph work, and failed/revoked write-lease
   cleanup beyond API cancel lease revocation. Product probe
   `961b0f47...` exposed
   ...
   readbacks. FR-11 is now validated by
   `tests/integration/test_graph_fr11_acceptance.py::test_fr11_scheduler_orders_frontier_and_progresses_deferred_work`
