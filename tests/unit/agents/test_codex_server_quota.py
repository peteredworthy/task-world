"""Unit tests for CodexServerRemoteAgent.get_quota() using FakeQuotaFetcher.
...
_API_KEY = "sk-test-codex-quota"  # pragma: allowlist secret


def _make_agent(api_key: str = _API_KEY) -> CodexServerRemoteAgent:
    """Construct a minimal CodexServerRemoteAgent with an explicit api_key."""
...
def test_missing_api_key_returns_none_without_calling_fetcher() -> None:
    """When the resolved token is absent, get_quota() returns None and the fetcher is not called."""
    ...
    agent = _make_agent()
    # Simulate a missing token by clearing _token after construction.
    agent._token = ""  # pyright: ignore[reportPrivateUsage]
