...
once, and writes validated query/fixture source updates plus the canonical
report during `--apply`.
...
## Second independent review-fix wave (post-`e4d4ddce7`)

### Assert-clean RED/GREEN

- **RED:** `test_query_migration_modes_check_apply_and_assert_clean_use_atomic_report`
  wrote the deterministic `--check` report into its temporary repository while
  leaving the source with `projection['run_state']`. Before the fix,
  `--assert-clean` incorrectly succeeded because it applied the pending query
  plan in memory before deriving the closure. The focused test failed with
  `Failed: DID NOT RAISE AnchorRefusedError`.
- **GREEN:** `run_query_migration_mode` now rejects `assert-clean` immediately
  when either `query_apply.updates` or `fixture_apply.updates` is nonempty,
  before `apply_source_updates_in_memory`, closure compilation, or report
  validation. The focused regression and full codemod unit module pass.

### Transaction finding evaluation

The requested crash-atomic all-path transaction is not added. The approved
protocol requires prevalidation, same-directory temporary files, source-mode
preservation, flush/fsync, then `os.replace`; the implementation follows that
sequence in `write_repository_migration_apply`: validates every destination,
stages and fsyncs every temporary file, and only then enters its per-destination
replacement loop. POSIX `rename`/`os.replace` is atomic per destination but
cannot make a multi-directory set process-crash atomic without a journal or
recovery protocol, neither of which the task specifies or needs.

The existing
`test_query_source_atomic_write_validates_every_original_before_writing` and
`test_fixture_mutation_atomic_write_validates_every_original_before_writing`
prove that a stale later source rejects the operation while the earlier
destination remains unchanged; together with the single writer's explicit
validate-all → stage-all/fsync → replace-all ordering, they prove no destination
replacement occurs before prevalidation/staging completes. They do not claim
impossible process-crash atomicity. No compatibility or rollback machinery was
introduced.
...
- `uv run python -m scripts.codemods.migrate_graph_projection_queries --assert-clean`
  — **passed: 754 sites**.
- `make test-graph-projection-migration` — **11 passed, 3,800 deselected**.
- Focused Ruff and format checks — passed; focused Pyright — **0 errors,
  0 warnings, 0 informations**.
- `make test` — **5,101 passed, 3 skipped, 3 Python 3.12 SQLite datetime
  deprecation warnings** in 111.31s.

Changed files: `scripts/codemods/migrate_graph_projection_queries.py`,
`tests/unit/test_migrate_graph_projection_queries.py`, and this report. No
consumer call sites were manually edited.
