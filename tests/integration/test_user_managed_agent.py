...
async def test_user_managed_agent_cross_instance(
    ...
    async def submit_via_different_service() -> None:
        # Yield one event-loop iteration so agent.execute() reaches its first
        # await and registers the submit event before we fire submit.
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        async with session_factory() as agent_session:
            ...
            async def submit_via_rest_api_and_drain() -> None:
                # Yield one event-loop iteration so agent.execute() reaches its
                # first await and registers the submit event before we fire it.
                await asyncio.sleep(0)
