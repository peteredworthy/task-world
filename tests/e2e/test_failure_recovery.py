...
async def test_multiple_runs_persist_independently(
    api_client: httpx.AsyncClient, drain: DrainFn
) -> None:
    ...
    await submit_task(api_client, run1_id, task_id, drain=drain)
    await grade_item(api_client, run1_id, task_id, "R1", grade="A")
    await complete_verification(api_client, run1_id, task_id, drain=drain)
...
@pytest.mark.e2e
async def test_task_attempts_persisted(api_client: httpx.AsyncClient, drain: DrainFn) -> None:
...
@pytest.mark.e2e
async def test_checklist_state_persisted(api_client: httpx.AsyncClient, drain: DrainFn) -> None:
...
@pytest.mark.e2e
async def test_run_list_reflects_persisted_state(
    api_client: httpx.AsyncClient, drain: DrainFn
) -> None:
    ...
    await submit_task(api_client, completed_run_id, task_id, drain=drain)
    await grade_item(api_client, completed_run_id, task_id, "R1", grade="A")
    await complete_verification(api_client, completed_run_id, task_id, drain=drain)
