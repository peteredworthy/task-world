...
| W1-REAL-1 | The repaired system is exercised through its real graph/Codex interface. | Two Wave 1 graph runs use Codex Server `gpt-5.6-luna` / high and record real callbacks or subsequent graph progress. | Original immutable 10,000-entry snapshots were paused; replacements snapshot the validated 50,000-entry policy without weakening secret/symlink checks. | Checkpoint `8e677613-36c1-4a96-a73e-46b5a678955b` accepted two patches; archival `b598c0d2-f8b5-46bd-b037-000ab4132414` recorded a real patch callback that the kernel safely rejected for a forbidden cycle while planning continued. | Live API/activity/graph evidence plus cache-boundary and terminal-failure regressions. | complete | None. |
...
- Both replacement runs use `codex_server`, `gpt-5.6-luna`, high reasoning,
  managed restrictions, REST callbacks, and an immutable cache scan budget of
  50,000 entries / 1 GiB.
