...
EVENT_PAYLOAD_SPECS: MappingProxyType[str, EventPayloadSpec] = MappingProxyType(
    {
        ...
        "node_created": _spec(
            "node_created",
            projection="access_mode access_mode_override_justification acceptance allowed_actions appealed_node_id approval_prompt approval_type artifacts attempt_number authority authority_request authority_request_record authority_request_record_id available_tools blocker blocker_reason bound_requirement_ids builder_agent cache_authority_hash candidate_id carryover_record_id check_index command_binding command_definition command_definition_id complexity context_source decision_request decision_request_record_id dynamic_feature execution_id failed_candidate_id fan_out gate gate_type generation_index guarded_planner_node_id hidden_oracle_command human_prompt id inputs invariants kind max_attempts mcp_servers membership message node_id objective outputs patch_id planner_chain planner_generation_budget preconditions priority profile prohibited_actions prompt reason recovery_of_node_id recovery_of_record_id recovery_reason region_label rejected_patch_id requirement requirement_id resource_claims role routine rubric run_id scope session_id snapshot state step_context step_id step_index submission_template task_context task_id task_index task_region_id title verifier_agent work_mode",
        ...
        "output_record_accepted": _spec(
            ...
            projection="attempt_number base_snapshot_id candidate_id evidence file_state_record_id outcome paths port producer_node_id record_id record_kind record_type schema snapshot_id supersedes_record_id supersedes_task_region_id supersedes_task_region_ids task_region_id value verdict",
