...
class GraphProjection(TypedDict):
    ...
    retry_not_before_by_node: dict[str, str]
    ...
    failed_verification_results: dict[str, dict[str, str]]
    passed_verification_results: dict[str, dict[str, str]]
    recovery_nodes_by_record_id: dict[str, list[dict[str, str]]]
    no_successor_patch_ids_by_node: dict[str, str]
...
def reduce_event(state: GraphProjection, event: EventEnvelope) -> GraphProjection:
    ...
    next_state: GraphProjection = {
        ...
        "latest_routine_snapshot_record": dict(latest_routine_snapshot_record)
        if latest_routine_snapshot_record is not None
        else None,
        "failed_verification_results": {
            record_id: dict(result)
            for record_id, result in state.get("failed_verification_results", {}).items()
        },
        "passed_verification_results": {
            record_id: dict(result)
            for record_id, result in state.get("passed_verification_results", {}).items()
        },
        "recovery_nodes_by_record_id": {
            record_id: [dict(node) for node in nodes]
            for record_id, nodes in state.get("recovery_nodes_by_record_id", {}).items()
        },
        "no_successor_patch_ids_by_node": dict(state.get("no_successor_patch_ids_by_node", {})),
        "approval_decisions": {
            node_id: dict(payload)
    ...
    if event.event_type == "run_lifecycle_changed":
        ...
        if isinstance(node_id, str):
            next_state["node_creation_positions"][node_id] = event.position
        ...
        if isinstance(planner_node_id, str) and isinstance(patch_id, str):
            ...
            if isinstance(successor_node_ids, list) and any(
                isinstance(item, str) for item in cast(list[Any], successor_node_ids)
            ):
                next_state["no_successor_patch_ids_by_node"].pop(planner_node_id, None)
            else:
                next_state["no_successor_patch_ids_by_node"][planner_node_id] = patch_id
        successor_node_ids = event.payload.get("successor_planner_node_ids")
        ...
        node_id = event.payload.get("node_id")
        retry_not_before = event.payload.get("retry_not_before")
        if isinstance(node_id, str):
            if isinstance(retry_not_before, str) and retry_not_before:
                next_state["retry_not_before_by_node"][node_id] = retry_not_before
            else:
                next_state["retry_not_before_by_node"].pop(node_id, None)
...
def _record_verification_result(state: GraphProjection, event: EventEnvelope) -> None:
    ...
    if not isinstance(node_id, str) or not node_id:
        return
    result: dict[str, str] = {"node_id": node_id, "record_id": record_id}
    ...
    if event.event_type == "verification_passed":
        state["passed_verification_results"][record_id] = result
        if isinstance(candidate_id, str):
            state["failed_verification_results"].pop(record_id, None)
    else:
        state["failed_verification_results"][record_id] = result
        if isinstance(candidate_id, str):
            state["passed_verification_results"].pop(record_id, None)
...
def _record_recovery_node(state: GraphProjection, event: EventEnvelope) -> None:
    ...
    if recovery_reason not in {"failed_required_check", "failed_verification"}:
        return
    nodes = state["recovery_nodes_by_record_id"].setdefault(cast(str, record_id), [])
    nodes.append(
        {
