...
async def test_detect_openhands_local_present(detected_options: list[AgentOption]) -> None:
    ...
    oh_local = [
        o for o in detected_options if o.agent_runner_type == AgentRunnerType.OPENHANDS_LOCAL
    ][0]
...
async def test_detect_openhands_docker_present(detected_options: list[AgentOption]) -> None:
    ...
    oh_docker = [
        o for o in detected_options if o.agent_runner_type == AgentRunnerType.OPENHANDS_DOCKER
    ][0]
...
async def test_detect_cli_tools_real(detected_options: list[AgentOption]) -> None:
    ...
    cli_options = [
        o for o in detected_options if o.agent_runner_type == AgentRunnerType.CLI_SUBPROCESS
    ]
...
async def test_user_managed_always_available(detected_options: list[AgentOption]) -> None:
    um = [o for o in detected_options if o.agent_runner_type == AgentRunnerType.USER_MANAGED][0]
...
async def test_detect_all_returns_both_openhands_types(detected_options: list[AgentOption]) -> None:
    ...
    agent_runner_types = [o.agent_runner_type for o in detected_options]
    assert AgentRunnerType.OPENHANDS_LOCAL in agent_runner_types
    assert AgentRunnerType.OPENHANDS_DOCKER in agent_runner_types
...
async def test_config_schema_populated(detected_options: list[AgentOption]) -> None:
    ...
    for option in detected_options:
        ...
        assert len(option.config_schema) > 0, (
            f"{option.name} ({option.agent_runner_type}) has empty config_schema"
        )
...
async def test_detect_codex_server_present(detected_options: list[AgentOption]) -> None:
    ...
    cs = [o for o in detected_options if o.agent_runner_type == AgentRunnerType.CODEX_SERVER]
    assert len(cs) == 1
    entry = cs[0]
...
async def test_detect_codex_server_config_fields(detected_options: list[AgentOption]) -> None:
    ...
    cs = [o for o in detected_options if o.agent_runner_type == AgentRunnerType.CODEX_SERVER][0]
...
async def test_detect_all_includes_codex_server_type(detected_options: list[AgentOption]) -> None:
    ...
    agent_runner_types = [o.agent_runner_type for o in detected_options]
    assert AgentRunnerType.CODEX_SERVER in agent_runner_types
