...
async def test_list_runs_empty(client: AsyncClient) -> None:
    # Use a unique repo name that no other test will create runs for — avoids
    # needing an empty DB even though client_and_drain is module-scoped.
    unique_repo = f"repo-never-used-{uuid.uuid4().hex[:8]}"
    response = await client.get(f"/api/runs?repo_name={unique_repo}")
    assert response.status_code == 200
    assert response.json() == {"runs": []}


async def test_list_runs(client: AsyncClient) -> None:
    unique_repo = f"repo-list-{uuid.uuid4().hex[:8]}"
    await client.post(
        "/api/runs",
        json={"routine_id": "simple-routine", "repo_name": unique_repo, "branch": "main"},
    )
    response = await client.get(f"/api/runs?repo_name={unique_repo}")
    assert response.status_code == 200
    data = response.json()
...
async def test_list_runs_filter_by_project(client: AsyncClient) -> None:
    unique_repo = f"repo-filter-{uuid.uuid4().hex[:8]}"
    await client.post(
        "/api/runs",
        json={"routine_id": "simple-routine", "repo_name": unique_repo, "branch": "main"},
    )
    ...
    response = await client.get("/api/runs?repo_name=other-project-never-created")
    assert response.status_code == 200
    assert len(response.json()["runs"]) == 0


async def test_list_runs_filter_by_status(client: AsyncClient) -> None:
    unique_repo = f"repo-status-{uuid.uuid4().hex[:8]}"
    await client.post(
        "/api/runs",
        json={"routine_id": "simple-routine", "repo_name": unique_repo, "branch": "main"},
    )

    response = await client.get(f"/api/runs?repo_name={unique_repo}&status=draft")
    assert response.status_code == 200
    assert len(response.json()["runs"]) == 1

    response = await client.get(f"/api/runs?repo_name={unique_repo}&status=active")
    assert response.status_code == 200
    assert len(response.json()["runs"]) == 0
...
async def test_list_runs_recent_hours(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        "/api/runs",
        json={"routine_id": "simple-routine", "repo_name": "repo-recent-hours", "branch": "main"},
    )
    assert resp.status_code == 201
    run_id = resp.json()["id"]

    # Run was just created — should appear in recent_hours=1 and recent_hours=24
    response = await client.get("/api/runs?recent_hours=1")
    assert response.status_code == 200
    ids = [r["id"] for r in response.json()["runs"]]
    assert run_id in ids, "Newly created run should appear in recent_hours=1 results"

    response = await client.get("/api/runs?recent_hours=24")
    assert response.status_code == 200
    ids = [r["id"] for r in response.json()["runs"]]
    assert run_id in ids, "Newly created run should appear in recent_hours=24 results"
