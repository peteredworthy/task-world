...
| 7 | Projection parity coverage for task-state fields | Summary/light reads cannot drift from full projection task-state semantics. | Fixed for corrective supersession; broader matrix still recommended | `tests/integration/test_graph_read_models.py::test_projection_read_model_preserves_corrective_supersession_task_states`. Compact graph payload fields include `supersedes_task_region_id(s)`. |
