...
2. FR-12/FR-16: investigate the post-resume `/run` and `/graph` readback
   ...
   heartbeat/renewal and no false `agent_died` before cancellation. Product run
   `a9b658fa...` proves the supported-runner artifact/output callback path, and
   `32be2af2...` proves real `agent_died` failure/retry callbacks when a worker
   exits without submit. Still open: a recovery re-entry path that actually
   exercises the stale-report guard with no duplicate-position/SQLite-lock
   failure, plus terminal exhausted-failure/`failure_record` callback behavior.
3. FR-08/FR-11/FR-16: run bounded product probes for authority
   ...
   fairness/starvation behavior, terminal failure-record callbacks, cleanup as
   explicit graph work, and failed/revoked write-lease cleanup beyond API cancel
   lease revocation. Product probe `961b0f47...` exposed
