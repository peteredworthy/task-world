...
def _semantic_final_gate_blockers(
    ...
    current_report_by_batch, ambiguous_batches = authoritative_batch_verification_report_ids(
        projection, declared_batches
    )
    active_node_ids = set(effective_active_node_ids_view(projection))
    payloads = {
        node_id: payload
    ...
    passed_batches = {
        batch_id
        for batch_id, record_id in current_report_by_batch.items()
        if record_id in gate_record_ids
    }
    batch_report_ids = {current_report_by_batch[batch_id] for batch_id in passed_batches}
    configured_batch_order = (
        [item for item in cast(list[Any], configured) if isinstance(item, str)]
        if isinstance(configured, list)
        else []
    )
    ordered_batch_report_ids = [
        current_report_by_batch[batch_id]
        for batch_id in configured_batch_order
        if batch_id in passed_batches
    ]
    expected_candidate_ids: list[str] = []
    expected_file_state_ids: list[str] = []
    for record_id in ordered_batch_report_ids:
        report = cast(VerificationReportRecord, records[record_id])
        for candidate_id in [
            *report.candidate_record_ids,
            *([report.candidate_record_id] if report.candidate_record_id is not None else []),
        ]:
            if candidate_id not in expected_candidate_ids:
                expected_candidate_ids.append(candidate_id)
        for file_state_id in report.file_state_record_ids:
            if file_state_id not in expected_file_state_ids:
                expected_file_state_ids.append(file_state_id)

    final_snapshot_id: str | None = None
    final_candidate_id: str | None = None
    if configured_batch_order and configured_batch_order[-1] in current_report_by_batch:
        final_report = cast(
            VerificationReportRecord,
            records[current_report_by_batch[configured_batch_order[-1]]],
        )
        final_candidate_id = final_report.candidate_id
        if len(final_report.file_state_record_ids) == 1:
            final_file_state = file_state_records_view(projection).get(
                final_report.file_state_record_ids[0]
            )
            if final_file_state is not None:
                final_snapshot_id = final_file_state.snapshot_id

    final_acceptance_record_ids = {
        record_id
        for record_id in gate_record_ids
        if isinstance(record := records.get(record_id), CheckResultRecord)
        and record.value.status == "passed"
        and payloads.get(record.producer_node_id, {}).get("semantic_stage") == "final_acceptance"
        and payloads.get(record.producer_node_id, {}).get("command_binding")
        == "dynamic_feature_acceptance"
        and (record.verification_report_record_ids or record.value.verification_report_record_ids)
        == ordered_batch_report_ids
        and (record.candidate_record_ids or record.value.candidate_record_ids)
        == expected_candidate_ids
        and (record.file_state_record_ids or record.value.file_state_record_ids)
        == expected_file_state_ids
        and record.candidate_id == final_candidate_id
        and final_snapshot_id is not None
        and record.value.execution_snapshot_id == final_snapshot_id
    }
    all_batch_report_ids = {
        record_id
        for record_id, record in records.items()
        if isinstance(record, VerificationReportRecord)
        and isinstance(
            (node := payloads.get(record.producer_node_id, {})).get("declared_batch_id"), str
        )
        and node.get("semantic_stage") in {"effectful_batch", "corrective_work"}
    }
    all_final_acceptance_record_ids = {
        record_id
        for record_id, record in records.items()
    ...
    final_audit_passed = any(
        ...
        and set(record.evaluated_record_ids) & all_batch_report_ids == batch_report_ids
        and len(final_acceptance_record_ids) == 1
        and set(record.evaluated_record_ids) & all_final_acceptance_record_ids
        == final_acceptance_record_ids
    ...
    final_acceptance_passed = bool(final_acceptance_record_ids)
    ...
    blockers.extend(
        {
            "kind": "ambiguous_declared_batch_verification",
            "batch_id": batch_id,
        }
        for batch_id in sorted(ambiguous_batches)
    )
    if not final_audit_passed:
        blockers.append({"kind": "missing_final_independent_audit"})
    if not final_acceptance_passed:
        blockers.append({"kind": "missing_dynamic_feature_acceptance"})
    return blockers


def authoritative_batch_verification_report_ids(
    projection: GraphProjection,
    declared_batches: set[str],
) -> tuple[dict[str, str], set[str]]:
    """Return the sole active, topology-valid report for each declared batch.

    Corrective reports supersede originals only when their exact failed-evidence
    lineage is valid. Acceptance execution and final completion share this query.
    """
    records = output_record_payloads_view(projection)
    active_node_ids = set(effective_active_node_ids_view(projection))
    payloads = {
    ...
    originals: dict[str, list[str]] = {batch_id: [] for batch_id in declared_batches}
    corrections: dict[str, list[str]] = {batch_id: [] for batch_id in declared_batches}
    for record_id, record in records.items():
        ...
        valid_batch_stage = producer.get("semantic_stage") == "effectful_batch" or (
            isinstance(batch_id, str)
            and _corrective_batch_report_supersedes_region(
                projection, record, batch_id, records, payloads
            )
        )
        if (
            valid_batch_stage
            ...
            target = (
                corrections if producer.get("semantic_stage") == "corrective_work" else originals
            )
            target[batch_id].append(record_id)
    current: dict[str, str] = {}
    ambiguous: set[str] = set()
    for batch_id in declared_batches:
        candidates = corrections[batch_id] or originals[batch_id]
        if len(candidates) == 1:
            current[batch_id] = candidates[0]
        elif len(candidates) > 1:
            ambiguous.add(batch_id)
    return current, ambiguous


def _corrective_batch_report_supersedes_region(
    projection: GraphProjection,
    ...
    verifier = payloads.get(report.producer_node_id, {})
    if (
        verifier.get("semantic_stage") != "corrective_work"
        or verifier.get("declared_batch_id") != batch_id
        or verifier.get("recovery_reason") != "failed_verification"
    ):
        return False
    ...
    for candidate_id in candidate_ids:
        ...
        if not isinstance(candidate, CandidateRecord):
            continue
        worker = payloads.get(candidate.producer_node_id, {})
        superseded_region_id = correction_superseded_task_region_id(projection, worker)
        if (
            superseded_region_id is not None
            and candidate.supersedes_task_region_id == superseded_region_id
            and worker.get("declared_batch_id") == batch_id
            and verifier.get("recovery_of_node_id") == worker.get("recovery_of_node_id")
            and verifier.get("recovery_of_record_id") == worker.get("recovery_of_record_id")
            and verifier.get("failed_candidate_id") == worker.get("failed_candidate_id")
        ):
            return True
    return False
...
def _output_record_contract_conflict(
    ...
    for index, raw_record in enumerate(typed_raw_records):
        ...
        if _is_candidate_record_payload(record_payload):
            ...
            supersession_conflict = _candidate_supersession_conflict(
                projection,
                node_payload,
                record_payload,
                index,
            )
            if supersession_conflict is not None:
                return supersession_conflict
    ...
    for raw_record in ordered_raw_records:
        ...
        if _is_candidate_record_payload(record_payload):
            ...
            superseded_region_id = correction_superseded_task_region_id(
                projection,
                node_payload_view(projection, expected_producer_node_id) or {},
            )
            record_payload.pop("supersedes_task_region_id", None)
            record_payload.pop("supersedes_task_region_ids", None)
            if superseded_region_id is not None:
                record_payload["supersedes_task_region_id"] = superseded_region_id
            try:
...
def _candidate_is_bound_to_verifier(
    ...
    candidate_ports = ["candidate_under_test", "semantic_artifact"]
    candidate_ports.extend(port for port in bindings if port.startswith("verification_report_"))
    for port in candidate_ports:
        binding = bindings.get(port)
        if binding is not None and candidate_id in binding.record_ids:
            return True
    node = node_payload_view(projection, verifier_node_id) or {}
    if node.get("semantic_stage") == "final_audit":
        cited_candidates = _evaluated_record_citations(projection, verifier_node_id).get(
            "candidate_record_ids", []
        )
        if candidate_id in cited_candidates:
            return True
    return False
...
def _candidate_supersession_conflict(
    projection: GraphProjection,
    ...
    supplied: list[str] = []
    singular = payload.get("supersedes_task_region_id")
    if singular is not None:
        if not isinstance(singular, str):
            return f"candidate record at index {index} has invalid supersession region"
        supplied.append(singular)
    plural = payload.get("supersedes_task_region_ids")
    if plural is not None:
        if not isinstance(plural, list) or not all(
            isinstance(item, str) for item in cast(list[Any], plural)
        ):
            return f"candidate record at index {index} has invalid supersession regions"
        supplied.extend(cast(list[str], plural))
    if not supplied:
        return None
    expected = correction_superseded_task_region_id(projection, producer)
    if expected is None or set(supplied) != {expected}:
        return (
            f"candidate record at index {index} supersession is not proven by its "
            "failed-verification correction lineage"
        )
    return None
...
def _evaluated_record_citation_conflict(
    ...
    try:
        ...
        return f"output record at index {index} has invalid evidence closure: {exc}"
...
def _evaluated_record_citations(
    ...
    return evidence_closure_for_node(projection, node_id, consumer=consumer).citations()
            ...
            GraphPatchAcceptedPayload.model_validate(
                {
                    ...
                    "operation_key": submit_patch_operation_key(payload),
                    "operation_fingerprint": submit_patch_operation_fingerprint(payload),
                }
            ).model_dump(mode="json"),
        )
    ]
...
def _stamp_reliable_plan_successor_authority(
    ...
    """Propagate bounded sequential authority to the next horizon planner."""
    parent = node_payload_view(projection, proposed_by_node_id) or {}
    skeleton_id = parent.get("reliable_plan_skeleton_id")
    raw_carrier = parent.get("reliable_plan_assignment_carrier")
    carrier = (
        ReliablePlanAssignmentCarrier.model_validate(raw_carrier)
        if isinstance(raw_carrier, dict)
        else None
    )
    if isinstance(skeleton_id, str) and carrier is None:
        raise ValueError(
            "reliable-plan execution node is missing sealed assignment carrier; "
            "recreate the run with a qualified assignment arm"
        )
    raw_remaining = parent.get("reliable_plan_remaining_horizons")
    remaining = (
        raw_remaining
    ...
    for op in ops:
        ...
        for key, value in op.items():
            copied[key] = dict(cast(Mapping[str, Any], value)) if isinstance(value, dict) else value
        stamped.append(copied)
    for op_name, node_key, node in iter_patch_node_payloads(stamped):
        default_kind = patch_node_default_kind(op_name, node_key)
        ...
        if isinstance(skeleton_id, str) and carrier is not None:
            if op_name == "create_revision_attempt" and node_key == "worker_node":
                node["semantic_stage"] = "corrective_work"
            assignment_role: ReliablePlanAssignmentRole = (
                "correction_worker"
                if op_name == "create_revision_attempt" and node_key == "worker_node"
                else _reliable_plan_assignment_role(
                    node,
                    default_kind=default_kind,
                )
            )
            assignment = carrier.assignment_for(assignment_role)
            node["reliable_plan_assignment_carrier"] = carrier.model_dump(mode="json")
            node["reliable_plan_assignment_role"] = assignment_role
            node["reliable_plan_selected_runner_type"] = carrier.selected_runner_type
            node["reliable_plan_skeleton_id"] = skeleton_id
            node["reliable_plan_qualification_evidence_hash"] = parent.get(
                "reliable_plan_qualification_evidence_hash"
            )
...
def _stamp_failed_verification_correction_lineage(
    projection: GraphProjection,
    ops: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Attach controller-owned failed-verification lineage to proven corrections."""
    worker_proofs: dict[str, tuple[str, str, str, str]] = {}
    records = output_record_payloads_view(projection)
    for op in ops:
        ...
        failed_region_id = correction_superseded_task_region_id(
            projection,
            node,
            require_stamped_lineage=False,
        )
        failed_record_id = node.get("failed_verification_record_id")
        if failed_region_id is None or not isinstance(failed_record_id, str):
            continue
        ...
        if not all(isinstance(value, str) for value in (node_id, task_region_id, candidate_id)):
            continue
        worker_proofs[cast(str, node_id)] = (
            report.producer_node_id,
            failed_record_id,
            cast(str, node.get("failed_candidate_id")),
            cast(str, node.get("declared_batch_id")),
        )

    verifier_proofs: dict[str, tuple[str, str, str, str]] = {}
    for op in ops:
        if op.get("op") != "create_edge":
            continue
        worker_id = op.get("from_node_id")
        verifier_id = op.get("to_node_id")
        proof = worker_proofs.get(worker_id) if isinstance(worker_id, str) else None
        if (
            proof is not None
            and isinstance(verifier_id, str)
            and op.get("from_port") == "candidate"
            and op.get("to_port") == "candidate_under_test"
        ):
            verifier_proofs[verifier_id] = proof
    ...
    for op in ops:
        ...
        if copied.get("op") == "create_node" and isinstance(raw_node, dict):
            node = dict(cast(dict[str, Any], raw_node))
            node_id = node.get("node_id")
            proof = (worker_proofs.get(node_id) if isinstance(node_id, str) else None) or (
                verifier_proofs.get(node_id) if isinstance(node_id, str) else None
            )
            if proof is not None:
                failed_node_id, failed_record_id, failed_candidate_id, batch_id = proof
                if (
                    node.get("semantic_stage") == "corrective_work"
                    and node.get("failed_candidate_id") == failed_candidate_id
                    and node.get("declared_batch_id") == batch_id
                ):
                    node["recovery_reason"] = "failed_verification"
                    node["recovery_of_node_id"] = failed_node_id
                    node["recovery_of_record_id"] = failed_record_id
                    copied["node"] = node
        output.append(copied)
    return output


def _reliable_plan_assignment_role(
    node: dict[str, Any],
    *,
    default_kind: str | None = None,
) -> ReliablePlanAssignmentRole:
    kind = node.get("kind", default_kind)
    ...
    if kind == "worker":
        ...
        if stage == "corrective_work" or role in {"correction_worker", "corrective_worker"}:
            return "correction_worker"
        if stage == "effectful_batch" or node.get("access_mode") == "write":
            return "implementation_worker"
        raise ValueError(
            f"reliable-plan worker {node.get('node_id', '<unknown>')} has ambiguous assignment role"
        )
    raise ValueError("reliable-plan assignment requested for non-model-backed node")
...
def _reliable_plan_successor_authority_rejection(
    ...
    if not isinstance(parent.get("reliable_plan_skeleton_id"), str):
        return None
    if (
        parent.get("semantic_stage") == "successor_planning"
        and reliable_plan_successor_horizon_materialized(projection, patch.proposed_by_node_id)
        and not successor_node_ids
        and not _patch_creates_reliable_plan_finalization(patch)
    ):
        return "reliable_plan_horizon_already_materialized"
    ...
    expected_horizon = (
        ...
        or (parent.get("role") == "gap_planner" and isinstance(parent.get("planning_horizon"), int))
        else 1
    )
    for op in patch.ops:
...
def _patch_creates_reliable_plan_finalization(patch: PatchEnvelope) -> bool:
    created = [op.node for op in patch.ops if op.op == "create_node" and isinstance(op.node, dict)]
    return (
        any(
        ...
        and any(node.get("kind") == "final_gate" for node in created)
        and any(
...
def _reliable_plan_recovery_planner_payload(
    projection: GraphProjection,
    payload: dict[str, Any],
) -> dict[str, Any]:
    """Carry sealed run authority into controller-created correction planners."""
    root = node_payload_view(projection, "root") or {}
    raw_carrier = root.get("reliable_plan_assignment_carrier")
    if not isinstance(raw_carrier, dict):
        return payload
    carrier = ReliablePlanAssignmentCarrier.model_validate(raw_carrier)
    assignment = carrier.assignment_for("successor_planner")
    return {
        **payload,
        ...
        "reliable_plan_one_horizon_authorized": root.get("reliable_plan_one_horizon_authorized"),
        "reliable_plan_remaining_horizons": root.get("reliable_plan_remaining_horizons"),
        "reliable_plan_qualification_evidence_hash": root.get(
            "reliable_plan_qualification_evidence_hash"
        ),
    }
    ...
    *,
    ...
    for failed_check in _current_failed_check_results(projection):
        if record_ids is not None and failed_check["record_id"] not in record_ids:
            continue
        # A batch verifier that consumes this exact failed check is the
        # deterministic semantic-recovery owner.  Let it emit the joined
        # failed verification report instead of racing a second check-owned
        # gap planner into existence.
        if _failed_check_has_verification_owner(projection, failed_check):
            continue
        ...
        recovery_payload = _reliable_plan_recovery_planner_payload(
            projection,
            {
...
def _passed_verification_terminalization_events(
    ...
    *,
    ...
    for verification in passed_verifications:
        ...
        if (
            verification == latest_passed_verification
            and not _verification_has_controller_owned_finalization(projection, verification)
        ):
            output.extend(
...
def _verification_has_controller_owned_finalization(
    projection: GraphProjection,
    verification: dict[str, str],
) -> bool:
    """Reliable-plan regions already carry their explicit acceptance and audit graph.

    The legacy terminalization sweep may synthesize a hidden-oracle check for
    unstructured graphs.  Doing that for any verifier in a reliable-plan
    skeleton incorrectly treats plan or intermediate-batch verification as the
    end of implementation and can leave a stale early check blocking the final
    gate.
    """
    node_id = verification.get("node_id")
    if not isinstance(node_id, str):
        return False
    node = node_payload_view(projection, node_id) or {}
    return isinstance(node.get("reliable_plan_skeleton_id"), str)
...
def _unreachable_failure_branch_node_ids(
    ...
    for edge in edges_view(projection).values():
        ...
        if edge.from_port != "verification_report":
            continue
        if edge.to_port not in {"verification_evidence", "verification_report"}:
            continue
...
def _selector_value_match(edge: EdgeProjection, key: str) -> Any:
    ...
    if not isinstance(selector, dict):
        return None
    options = selector.get("selectors")
    if selector.get("record_type") == "any_of" and isinstance(options, list):
        matches = {
            value
            for option in cast(list[Any], options)
            if isinstance(option, dict)
            and (value := _selector_terminal_value(cast(dict[str, Any], option), key)) is not None
        }
        return next(iter(matches)) if len(matches) == 1 else None
    return _selector_terminal_value(selector, key)


def _selector_terminal_value(selector: dict[str, Any], key: str) -> Any:
...
def _failed_check_has_verification_owner(
    projection: GraphProjection,
    failed_check: dict[str, str],
) -> bool:
    """Whether an exact live verifier is already responsible for this check."""
    record = output_record_payloads_view(projection).get(failed_check["record_id"])
    if not isinstance(record, CheckResultRecord):
        return False
    record_payload = record.model_dump(mode="json")
    for edge in edges_view(projection).values():
        if (
            edge.from_node_id != failed_check["node_id"]
            or edge.from_port != "check_result"
            or node_kinds_view(projection).get(edge.to_node_id) != "verifier"
            or node_states_view(projection).get(edge.to_node_id)
            in {"cancelled", "failed", "retired"}
            or not record_selector_matches(edge.accepted_record_selector, record_payload)
        ):
            continue
        candidate_binding = (
            input_bindings_view(projection).get(edge.to_node_id, {}).get("candidate_under_test")
        )
        verifier_payload = node_payload_view(projection, edge.to_node_id) or {}
        if verifier_payload.get("candidate_id") == record.candidate_id or (
            candidate_binding is not None and record.candidate_id in candidate_binding.record_ids
        ):
            return True
    return False
...
def _apply_agent_died(
    ...
    declared_node_payload = node_payload_view(projection, node_id)
    max_attempts = effective_node_max_attempts(
        node_kinds_view(projection).get(node_id),
        declared_node_payload.get("max_attempts")
        if declared_node_payload is not None
        else payload.max_attempts,
    )
    if max_attempts is None:
        max_attempts = payload.max_attempts
    elif payload.max_attempts > 0:
        # A recovery caller may tighten a legacy/default budget, but can never
        # enlarge the node's durable execution budget.
        max_attempts = min(max_attempts, payload.max_attempts)
    attempt_number = effective_node_attempt_number(node_attempts_view(projection).get(node_id))
...
def _patch_op_events(
    ...
    *,
    ...
    if op.op == "create_revision_attempt":
        ...
        worker_event = _node_created_event(projection, make_event, worker_node)
        verifier_event = _node_created_event(projection, make_event, verifier_node)
        revision_node = {
        ...
        return [
            make_event(
                ...
                {
                    "node": revision_node,
                    "worker_node": worker_event.payload,
                    "verifier_node": verifier_event.payload,
                },
            ),
            worker_event,
            verifier_event,
