...
| 9 | `claude_sdk` graph submit fix | `claude_sdk` can submit graph callbacks through the same path as the reliable graph runner, or is explicitly gated off with a reproduction-backed reason. | Fixed | `tests/unit/test_claude_sdk_agent.py::TestBuildOrchestratorMcpServer::test_submit_graph_patch_accepts_object_patch_through_mcp_handler` preserves the adjacent SDK MCP schema fix. `tests/integration/test_graph_run_driver.py::test_driver_rejects_unsupported_graph_runner_before_seeding[claude_sdk-claude_sdk]` locks the closure path from the HTML review: `claude_sdk` graph runs are explicitly gated off before graph seeding with `graph_runner_unsupported`, leaving `codex_server` as the reliable graph runner until the SDK stream lifecycle failure has a reproduction-backed fix. |
