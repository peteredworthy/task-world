...
- `GET /api/agent-runners/{type}/profiles` response: current profile-to-model mapping
...
- Save triggers `PUT /api/agent-runners/{type}/profiles` to persist to backend
...
2. Fetch profile data from `GET /api/agent-runners/{type}/profiles` on card load
3. Wire save to `PUT /api/agent-runners/{type}/profiles` endpoint
...
- Profile API: `GET /api/model-profiles`, `GET/PUT /api/agent-runners/{type}/profiles`
