...
def create_local(
    ...
    **kwargs: Any,
    ...
    """Create an OpenHandsAgent (local) from agent_runner_config.

    Import is deferred to avoid pulling in optional ``openhands`` dependency
    at module load time.

    Args:
        agent_runner_config: Configuration dict from the run.
    ...
    model = agent_runner_config.get("model", "gpt-5-mini")
    max_iterations = int(agent_runner_config.get("max_iterations", 100))
    max_actions = int(agent_runner_config.get("max_actions", 200))
    llm_config = coerce_llm_config(agent_runner_config)
...
def create_docker(
    ...
    **kwargs: Any,
    ...
    """Create a DockerOpenHandsAgent from agent_runner_config.
    ...
    model = agent_runner_config.get("model", "gpt-5-mini")
    max_iterations = int(agent_runner_config.get("max_iterations", 100))
    server_image = agent_runner_config.get("server_image")
    llm_config = coerce_llm_config(agent_runner_config)
