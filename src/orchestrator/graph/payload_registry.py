...
EVENT_PAYLOAD_SPECS: MappingProxyType[str, EventPayloadSpec] = MappingProxyType(
    {
        ...
        "node_created": _spec(
            "node_created",
            projection="allowed_actions appealed_node_id approval_prompt approval_type artifacts attempt_number authority authority_request authority_request_record authority_request_record_id available_tools blocker blocker_reason builder_agent cache_authority_hash candidate_id carryover_record_id check_index command_binding command_definition command_definition_id complexity context_source decision_request decision_request_record_id dynamic_feature execution_id failed_candidate_id fan_out gate gate_type generation_index guarded_planner_node_id hidden_oracle_command human_prompt id inputs kind max_attempts mcp_servers membership message node_id outputs patch_id planner_chain planner_generation_budget preconditions priority profile prompt reason recovery_of_node_id recovery_of_record_id recovery_reason region_label rejected_patch_id requirement requirement_id resource_claims role routine rubric run_id session_id snapshot state step_context step_id step_index submission_template task_context task_id task_index task_region_id title verifier_agent work_mode",
