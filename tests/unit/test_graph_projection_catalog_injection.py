...
def test_reduce_event_honors_the_injected_catalog() -> None:
    lifecycle_event = EventEnvelope(
        ...
        payload={"from_state": "pending", "to_state": "active", "reason": "start"},
    )

    empty_catalog = GraphCatalog.compose((), ())
    empty_result = reduce_event(empty_catalog, initial_projection(), lifecycle_event)
    normal_result = reduce_event(build_graph_catalog(), initial_projection(), lifecycle_event)

    assert empty_result["run_state"] is None
