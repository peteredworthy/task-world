...
describe('AgentQuotaBadge', () => {
  ...
  it('renders dollar format for balance_usd', () => {
    render(<AgentQuotaBadge quota={makeQuota({ balance_usd: 5.42 })} />);
    expect(screen.getByText('$5.42')).toBeInTheDocument();
  });

  it('renders percentage format for balance_pct', () => {
    render(<AgentQuotaBadge quota={makeQuota({ balance_pct: 35 })} />);
    expect(screen.getByText('35%')).toBeInTheDocument();
  });

  it('shows balance_usd when both balance_usd and balance_pct are set', () => {
    render(<AgentQuotaBadge quota={makeQuota({ balance_usd: 5.42, balance_pct: 35 })} />);
    expect(screen.getByText('$5.42')).toBeInTheDocument();
  ...
  it('applies green colour for balance_pct = 60', () => {
    render(<AgentQuotaBadge quota={makeQuota({ balance_pct: 60 })} />);
  ...
  it('applies red colour for balance_pct = 10', () => {
    render(<AgentQuotaBadge quota={makeQuota({ balance_pct: 10 })} />);
  ...
  it('applies green when max_balance_usd is null or 0 (no ratio / division-by-zero guard)', () => {
    const { unmount } = render(<AgentQuotaBadge quota={makeQuota({ balance_usd: 5 })} />);
    expect(screen.getByText('$5.00')).toHaveClass('bg-green-100');
    unmount();
    render(<AgentQuotaBadge quota={makeQuota({ balance_usd: 5, max_balance_usd: 0 })} />);
    expect(screen.getByText('$5.00')).toHaveClass('bg-green-100');
  });
  ...
  it('title attribute equals quota.label', () => {
    render(<AgentQuotaBadge quota={makeQuota({ balance_pct: 60, label: 'OpenAI credits' })} />);
