...
- **Cost rate configuration** -- new `model_costs.yaml` file and `runners/costs.py` loader providing `get_model_costs(model_name)`. Ships with rates for Sonnet, Haiku, and Opus. Unknown models default to zero rates. [I-04]
...
- **Run-level aggregation** -- accumulate per-model usage across all attempts/tasks/steps into `Run.token_usage_by_model`. [I-06]
...
- **API exposure** -- `ModelTokenUsageSchema` added to `AttemptSchema` and `RunResponse`. Frontend receives the full per-model array with embedded rates and computed `total_cost_usd`. [I-09]
- **Frontend display** -- per-model breakdown table on the run detail page. Total cost computed client-side by summing `total_cost_usd` across models. No model-to-cost mapping needed in frontend code. [I-10]
...
2. `model_costs.yaml` ships with Sonnet, Haiku, and Opus rates; `runners/costs.py` loads it and returns zero-rate defaults for unknown models. [I-17]
...
| Model name strings may differ across runner types | Mismatched keys in cost lookup | Normalize model names in `get_model_costs()`; unknown models get zero rates with "cost unknown" badge [I-04, I-17] |
