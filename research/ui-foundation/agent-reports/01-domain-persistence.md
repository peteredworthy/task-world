...
|---|---|---|---|
| DP-ENT-run-config | `RoutineConfig` is a versionable template shape with `id`, nested `StepConfig` and `TaskConfig`, but is not the persisted run instance. | implemented, tested | `src/orchestrator/config/models.py::RoutineConfig`; `tests/unit/test_run_factory.py::simple_routine` |
...
| DP-ENT-event | The authoritative scoped event row is `EventV2Model`, identified for global ordering by integer `position` and for per-aggregate duplicate/order protection by `(aggregate_id, version)`. It deliberately has no `event_id` column. | implemented, tested | `src/orchestrator/db/orm/models.py::EventV2Model`; `src/orchestrator/db/access/event_store_v2.py::StoredEvent`, `SqliteEventStore`; `tests/integration/test_database.py::test_event_v2_metadata_exposes_durability_contract`, `test_events_v2_retry_identity_is_aggregate_version_not_payload_identity` |
...
| `events_v2` | PK global `position`; unique `(aggregate_id, version)`; raw JSON text payload. | authoritative event-store row per implementation docstrings; no run FK |
...
| `jobs.J5.artifacts` | Interaction-log artifacts are typed rows; expected artifacts/evidence files are paths; graph artifact-like outputs are embedded records. No single artifact inventory or shared artifact ID exists. | partial, tested in separate paths; umbrella identity unclear |
...
- **untested in inspected evidence:** distinct non-null values for
  ...
  selection identity, or relational integrity from node/record/requirement IDs
  to core rows.
...
- **tested evidence:** event-store wiring/durability tests verify DB and JSONL
  positions and projection rebuild from `events_v2`.
- **Disposition:** unresolved documentation conflict, but current scoped
  implementation establishes SQL-first authority. Later catalogs must preserve
  the conflicting documentation rather than describing JSONL-first behavior as
  current.
...
4. Reconcile the SQL-first event authority implementation with JSONL-first
   project documentation and update the authoritative architecture statement in
   a separate task.
...
- `src/orchestrator/db/access/event_outbox.py::commit_with_event_outbox` and
  `src/orchestrator/db/access/jsonl_outbox.py::drain_committed_events_to_journal`.
...
5. Have tests/documentation audit register DP-CON-02, DP-CON-04, and DP-CON-05
   rather than smoothing them into architecture prose.
