...
def test_agent_info_type() -> None:
    ...
    assert agent.info.agent_runner_type == AgentRunnerType.RETIRED
...
async def test_execute_raises_agent_not_available_when_sdk_not_installed() -> None:
    ...
    try:
        ...
        assert exc_info.value.agent_runner_type == AgentRunnerType.RETIRED.value
    finally:
...
def test_claude_sdk_agent_uses_retired_runner_type() -> None:
...
def test_claude_sdk_is_distinct_from_other_types() -> None:
    ...
    assert AgentRunnerType.RETIRED not in other_types
