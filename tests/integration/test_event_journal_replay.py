...
async def _migrated_db_template(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """Run Alembic migrations once and return a template DB path.

    Copying this template is ~10ms vs running migrations (~400ms) for each
    test that needs a file-backed database.
    """
    tmpdir = tmp_path_factory.mktemp("db_template")
    db_path = tmpdir / "template.db"
    app = create_app(
        db_path=str(db_path),
