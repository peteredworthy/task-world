...
def run_metrics(run_id: str, fetcher: Fetcher = _get) -> dict[str, Any]:
    ...
    return {
        ...
        "attempts": attempts,
        "retries": retries,
        "gen_ai_usage_input_tokens": run.get("total_tokens_read") or 0,
        "gen_ai_usage_output_tokens": run.get("total_tokens_write") or 0,
        "gen_ai_usage_cache_read_input_tokens": run.get("total_tokens_cache") or 0,
...
def aggregate_bucket(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "avg_tokens_read": sum(r["gen_ai_usage_input_tokens"] for r in rows) / div,
        "avg_tokens_write": sum(r["gen_ai_usage_output_tokens"] for r in rows) / div,
        "avg_tokens_cache": sum(r["gen_ai_usage_cache_read_input_tokens"] for r in rows) / div,
